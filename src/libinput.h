 ...
 * LIBINPUT_TABLET_PROXIMITY_OUT, LIBINPUT_EVENT_TABLET_TOOL_UPDATE and
 * LIBINPUT_EVENT_TABLET_BUTTON.
 */
struct libinput_event_tablet;
 ...
 * @ingroup event_tablet
 *
 * Checks if an axis was updated in the last event or return 0 otherwise.
 ...
 * - LIBINPUT_TABLET_AXIS_X and LIBINPUT_TABLET_AXIS_Y - the raw X and Y
 *   cooirdinates of the tablet tool. By default these are not transformed,
 *   however libinput provides @ref libinput_event_tablet_get_x_transformed and
 *   @ref libinput_event_tablet_get_y_transformed for transforming each
 *   respective axis value
 * - LIBINPUT_TABLET_AXIS_DISTANCE - Approximately how many millimeters away
 *   from the tablet's sensor the tool is
 * - LIBINPUT_TABLET_AXIS_PRESSURE - The current pressure being applied on the
 *   tool in use, normalized from 0 to 1
 * - LIBINPUT_TABLET_AXIS_TILT_VERTICAL and
 ...
 * function returns 0.
 *
 * @note It is an application bug to call this function for events other then
 * LIBINPUT_EVENT_TABLET_BUTTON.
 *
 * @param event The libinput tablet event
 * @return the button triggering this event
 ...
 * being used. For tools where this isn't supported, 0 will be returned.
 *
 * @param tool The libinput tool

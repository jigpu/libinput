...
struct libinput_event_tablet {
	...
	uint32_t time;
	double *axes;
	unsigned char changed_axes[NCHARS(LIBINPUT_TABLET_AXIS_CNT + 1)];
...
libinput_event_tablet_get_axis_value(struct libinput_event_tablet *event,
				     enum libinput_tablet_axis axis)
{
	return (axis <= LIBINPUT_TABLET_AXIS_CNT) ? event->axes[axis] : 0;
}
			    ...
			    uint32_t time)
{
	...
	/* Mark all the stylus buttons as released */
	for (int32_t b = BTN_TOUCH; b <= BTN_STYLUS2; b++) {
		update_seat_button_count(device->seat,
					 b,
